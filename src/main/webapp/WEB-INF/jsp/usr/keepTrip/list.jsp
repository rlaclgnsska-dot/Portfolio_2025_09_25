...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
	...
	<script>
		...
		function sendSelectedTrips() {
		    ...
		    document.querySelector('form[action="doKeepTripDelete"]').submit();
		}
	...
	<section class="mt-8 text-xl">
		<div class="container mx-auto px-3">
			<form action="doKeepTripDelete" method="post">
				<div class="flex container md-2 text-base">
					<div class="flex items-center"><span>총 : ${getKeepTripsCnt }개</span></div>
					<div class="flex-grow"></div>
					<a class="btn btn-outline btn-sm" href="write">새 글 작성</a>
					<button type="button" class="btn btn-outline btn-sm" onclick="sendSelectedTrips()">삭제</button>
				</div>
				...
				<div class="table-box-type mt-3">
					<table>
						...
						<tbody>
							<c:forEach var="keepTrip" items="${keepTrips }">
								<tr>
									<td>
										<input type="checkbox" name="tripCheckList" value="${keepTrip.id }" />
									</td>
									<td>${keepTrip.id }</td>
									<td>${keepTrip.boardName }</td>
									<td>${keepTrip.provinceCity }</td>
	 								<td class="hover:underline">
										<a href="../trip/detail?id=${keepTrip.id }">
											${keepTrip.placeName }
										</a>
									</td>
									<td>${keepTrip.price }</td>
									<td>${keepTrip.nickname }</td>
									<td>${keepTrip.regDate }</td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
